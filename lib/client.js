        ...
        switch (dp[2]) {
            ...
            case ('screenon'):
                if (state.val) {
                    ...
                    adapter.setState('screenon', false, false);
                }
                break;
            case ('screenoff'):
                if (state.val) {
                    send2wiobrowser('screenoff|true');
                    adapter.setState('screenoff', false, false);
                }
                break;
            case ('close'):
                if (state.val) {
                    send2wiobrowser('close|true');
                    adapter.setState('close', false, false);
                }
                break;
            case ('command'):
                const newstr = state.val.replace(' ', '|');
                send2wiobrowser('command|' + newstr);
                break;
            case ('web'):
                switch (dp[2]) {
                    case ('sendURL'):
                        send2wiobrowser('sendUrl|' + state.val);
                        adapter.setState('web.error', false, false);
                        break;
                    case ('zoom'):
                    ...
                    case ('slide'):
                        if (state.val) {
                            if (connected) {
                                ...
                                adapter.setState('web.slide', false, false);
                            }
                        } else {
                            clearInterval(intervalObj);
                        }
