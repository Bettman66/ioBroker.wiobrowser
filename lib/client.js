...
function wioBrowserClient(adapter) {
        ...
        switch (dp[2]) {
            ...
            case ('screenon'):
                if (state.val) {
                    ...
                    send2wiobrowser('screenon|false');
                }
                break;
            case ('slidetime'):
                if (state.val > 9) {
                    clearInterval(intervalObj);
                    intervalObj = setInterval(slideshow, state.val * 1000);
                } else {
                    clearInterval(intervalObj);
                }
                break;
    ...
    function slideshow() {
        if (pages.length > 0) {
            ...
            adapter.log.debug(pages.length);
            index = index + 1;
                ...
                switch (obj.TYP) {
                    ...
                    case ('EVENT'):
                        if (obj.EVENT == 'GOTFOCUS') {
                            adapter.getState('slidetime', function (err, state) {
                                if (state.val > 0) adapter.setState('slidetime', 0, true);
                            });
                        }
                        break;
                }
