    ...
    function send2Server(id, state) {
        ...
        switch (dp[2]) {
            ...
            case ('slide'):
                ...
                break;
            case ('message'):
                if (messages.length > style) {
                send2wiobrowser('message|' + messages[style].titel + '|' + state.val + '|' +
                    messages[style].time + '|' + messages[style].left + '|' + messages[style].top + '|' + messages[style].color);
                }
                break;
            case ('messagenumber'):
                if (messages.length > state.val) {
                send2wiobrowser('message|' + messages[state.val].titel + '|' + messages[state.val].caption + '|' +
                    messages[state.val].time + '|' + messages[state.val].left + '|' + messages[state.val].top + '|' + messages[state.val].color);
                }
                break;
            case ('messagestyle'):
                if (style < (messages.length - 1)) {
                    style = state.val;
                } else {
                    style = 0;
                }
