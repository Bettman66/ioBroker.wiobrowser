...
function ioBrowserClient(adapter) {
    if (!(this instanceof ioBrowserClient)) return new ioBrowserClient(adapter);
    ...
    (function _constructor(config) {
            ...
            if (!obj || !obj.common || obj.common.type !== 'boolean') {
                obj = {
                    ...
                    common: {
                        ...
                        name: 'If connected to ioBrowser',
                        type: 'boolean',
                        read: true,
                        write: false,
                        def: false
        ...
        client.on('data', function (data) {
            try {
                ...
                var dp = data.toString('utf8').split('|');
                adapter.log.debug(dp[0]);
                adapter.log.debug(dp[1]);
                switch (dp[0]) {
                    case ('url'):
                        adapter.setState('receiveURL', dp[1], true);
                        break;
                }
            } catch (err) {
...
module.exports = ioBrowserClient;
